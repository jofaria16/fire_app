...
st.set_page_config(
    ...
    page_icon="💰",
    layout="wide",  # melhor para mobile-friendly
...
if not st.session_state.acesso_permitido:
    ...
    codigo = st.text_input("", type="password")
    if st.button("Entrar"):
        if codigo == "1214":
    ...
    # --- Título ---
    st.title("💰 FIRE App Mobile")
    st.markdown("App clean e interativa para telemóvel: Património, Poupança, Valor Intrínseco e Buffett 2.0")

    # --- Menu ---
    menu = st.radio("Escolhe a função:", ["📊 Património", "💵 Poupança", "💹 Valor Intrínseco", "📈 Faria Buffett 2.0"], horizontal=True)
    ...
    if menu == "📊 Património":
        ...
        for i, row in df_patrimonio.iterrows():
            ...
            with col1:
                st.write(f"**{row['Conta']}**")
            with col2:
                valor = st.number_input("", value=float(row['Valor']), key=f"pat_{i}")
        ...
        if ticker:
            try:
                ...
                if preco_atual == 0 or eps == 0:
                    ...
                    with col3:
                        delta_text = f"{margem:.2f}%"
                        st.metric("Margem Segurança", delta_text)
                    if margem >= 20:
